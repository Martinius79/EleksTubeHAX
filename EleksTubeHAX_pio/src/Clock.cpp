...
uint32_t RtcGet()
{
  DBG_VERBOSE("RtcGet() returns RTC.get(): ");
  DBG_VERBOSE_L(RTC.get());
  return RTC.get();
}
void RtcSet(uint32_t tt)
{
...
void Clock::begin(StoredConfig::Config::Clock *config_)
{
  ...
  if (config->is_valid != StoredConfig::valid)
  {
    ...
    setTwelveHour(false);        // default is 24 hour format
    setBlankHoursZero(false);    // default is to show leading zeros
    setTimeZoneOffset(1 * 3600); // default is Central European Time (CET)
    setActiveGraphicIdx(1);      // default is the first clock face
...
time_t Clock::syncProvider()
{
  ...
  if (millis() - millis_last_ntp > refresh_ntp_every_ms || millis_last_ntp == 0)
  {
    ...
    DBG_VERBOSE("No WiFi, using RTC time: ");
    DBG_VERBOSE_L(rtc_now);
    return rtc_now;
  }
