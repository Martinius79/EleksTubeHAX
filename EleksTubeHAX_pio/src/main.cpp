...
void loop()
{
  ...
  if (lastMqttCommandExecuted != -1)
  {
    if (((millis() - lastMqttCommandExecuted) > (MQTT_SAVE_CONFIG_AFTER_SEC * 1000)) && menu.getState() == Menu::idle)
    {
      ...
      DBG_MQTT_L("");
      DBG_MQTT("Saving config! Triggered from MQTT command received...");
      stored_config.save();
      DBG_MQTT_L("Done!");
    }
  }
    ...
    else
    {
      ...
      else if (menu_state == Menu::utc_offset_hour)
      {
        ...
        if (menu_change != 0)
        {
          ...
          if (newOffset > 43200)
          { // overflow (12*3600 = 43200 -> subtract 24 hours)
            newOffset = newOffset - 86400;
          }
          else
          {
            if (newOffset < -43200)
            { // underflow (-12*3600 = -43200 -> add 24 hours)
              newOffset = newOffset + 86400;
            }
          }
  ...
  { // we have free time, spend it for loading next image into buffer
    ...
    if (time_in_loop < 20)
    { // we have still free time, spend it for MQTT loop and DST checking
#ifdef MQTT_ENABLED
      MqttLoopInFreeTime();
#endif
  ...
  else
  {
    ...
    Serial.println(time_in_loop); // print the time spent in the loop
  }
#endif
}
...
{ // check the actual hour is in the defined "night time"
  if (DAY_TIME_START < NIGHT_TIME_START)
