...
void RX8025::setRtcTime(uint8_t s, uint8_t m, uint8_t h, uint8_t w, uint8_t d, uint8_t mh, uint8_t y)
{
    ...
    Wire1.beginTransmission(RX8025_address);
    Wire1.write((byte)0x00);
    Wire1.write(decToBcd(s));
    Wire1.write(decToBcd(m));
    Wire1.write(decToBcd(h));
    Wire1.write(WeekToBdc(w));
    Wire1.write(decToBcd(d));
    Wire1.write(decToBcd(mh));
    Wire1.write(decToBcd(y));
    // 停止与从机的数据传输
...
byte RX8025::getData(byte regaddr)
{
    ...
    Wire1.endTransmission();
    // 由主设备用来向从设备请求字节。
    Wire1.requestFrom(RX8025_address, 1);
    // 读取数据
    return Wire1.read();
}
...
void RX8025::RX8025_init(void)
{
    ...
    {
        Wire1.write(RX8025_Control[i]);
    }
    // 使用指定的地址开始向I2C从设备进行传输。
    Wire1.endTransmission();
}

void RX8025::RX8025_init(uint32_t rtcSDA, uint32_t rtcSCL)
{
#ifdef DEBUG_VERBOSE
    Serial.println("DEBUG_VERBOSE: Entering RX8025_init(uint32_t, uint32_t)");
    Serial.print("DEBUG_VERBOSE: Initializing Wire1 with SDA = ");
    Serial.print(rtcSDA);
    Serial.print(", SCL = ");
    Serial.println(rtcSCL);
#endif

    // Initialize Wire1 with specified SDA and SCL pins.
    Wire1.begin(rtcSDA, rtcSCL);
#ifdef DEBUG_VERBOSE
    Serial.println("DEBUG_VERBOSE: Wire1.begin() called successfully.");
#endif

    // Begin I2C transmission to RX8025 RTC device.
    Wire1.beginTransmission(RX8025_address);
#ifdef DEBUG_VERBOSE
    Serial.print("DEBUG_VERBOSE: Beginning I2C transmission to address 0x");
    Serial.println(RX8025_address, HEX);
#endif

    // Send the initialization command.
    Wire1.write(0xe0);
#ifdef DEBUG_VERBOSE
    Serial.println("DEBUG_VERBOSE: Sent initialization command 0xE0.");
#endif

    // Send control bytes from RX8025_Control array.
    for (unsigned char i = 0; i < 2; i++)
    {
        Wire1.write(RX8025_Control[i]);
#ifdef DEBUG_VERBOSE
        Serial.print("DEBUG_VERBOSE: Sent control byte ");
        Serial.print(i);
        Serial.print(": 0x");
        Serial.println(RX8025_Control[i], HEX);
#endif
    }

    // End the transmission and capture the error code.
    uint8_t err = Wire1.endTransmission();
#ifdef DEBUG_VERBOSE
    Serial.print("DEBUG_VERBOSE: endTransmission() returned error code: ");
    Serial.println(err);
    Serial.println("DEBUG_VERBOSE: Exiting RX8025_init()");
#endif
}
...
long RX8025::getUnixtime()
{
    ...
    Wire1.endTransmission();
    // 由主设备用来向从设备请求字节。
    Wire1.requestFrom(RX8025_address, 7);
    // 读取数据
    uint16_t ss = bcdToDec(Wire1.read() & 0x7F);
    uint16_t mm = bcdToDec(Wire1.read() & 0x7f);
    uint16_t hh = bcdToDec(Wire1.read() & 0x3f);
    Wire1.read();
    uint16_t d = bcdToDec(Wire1.read() & 0x3f);
    uint16_t m = bcdToDec(Wire1.read() & 0x1f);
    uint16_t y = bcdToDec(Wire1.read() & 0xff) + 2000;
