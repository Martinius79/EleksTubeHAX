...
void nau88c22_init() {
    ...
    // === PLL from BCLK ===
    // BCLK = 48kHz * 16bit * 2ch = 1.536MHz (or 32bit slot: 3.072MHz)
    // Fout = 12.288MHz → N = 8 (1.536MHz) or N = 4 (3.072MHz)
    // Try N=8 first

    // PLL OFF
    nau_write(0x01, nau_read(0x01) & ~0x20);

    // PLL_N: N=8, MCLK_DIV2=0
    nau_write(0x24, 0x0008);

    // PLL_K1, K2, K3 = 0 (no fractional part)
    nau_write(0x25, 0x0000);
    nau_write(0x26, 0x0000);
    nau_write(0x27, 0x0000);

    // Clocking: CLKM=PLL, MCLKSEL=0 (div1), slave
...
void setup() {
    Serial.begin(115200);
    delay(2000);
    Serial.println("\n\nNAU88C22 Audio Test v5 - PLL from BCLK");
